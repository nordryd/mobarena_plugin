...
import static com.nordryd.mobarena.command.PluginCommand.MASTER_CMD_PREFIX;
import static com.nordryd.mobarena.command.PluginCommand.validateCommand;
...
public class CommandManager extends EventListener implements CommandExecutor, TabCompleter
{
     ...
     */
    public CommandManager(final JavaPlugin jPlugin) {
        super(jPlugin);
    }

    @Override
    public boolean onCommand(final CommandSender sender, final Command issuedCommand, final String label,
            final String... vargs) {
        if (!((sender instanceof Player) && MASTER_CMD_PREFIX.equalsIgnoreCase(label))) {
            return false;
        }

        final Player player = (Player) sender;
        final Optional<PluginCommand> command = validateCommand(((vargs.length == 0) ? "" : vargs[0]), sender,
                vargs.length);
    ...
    @Override
    public List<String> onTabComplete(final CommandSender sender, final Command issuedCommand, final String label,
            final String... vargs) {
        if (sender instanceof Player) {
            if (MASTER_CMD_PREFIX.equalsIgnoreCase(issuedCommand.getName())) {
